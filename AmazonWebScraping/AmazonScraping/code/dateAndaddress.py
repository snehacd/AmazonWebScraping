#The below code is to scrap data from multiple pages and with that its scraping date and address 
import requests
...
def scrape_laptop_data(product_url):
    ...
    if response.status_code == 200:
        ...
        sku_id = sku_id_element.get_text(strip=True) if sku_id_element else 'SKU ID not found'
        ...
        category = category_element.get_text(strip=True) if category_element else 'Category not found'

        mrp_element = soup.find('span', class_='a-price a-text-price')
        mrp_text = mrp_element.get_text(strip=True) if mrp_element else 'MRP not found'
        ...
        selling_price = selling_price_element.get_text(strip=True) if selling_price_element else 'Selling Price not found'
        ...
        discount = discount_element.get_text(strip=True) if discount_element else 'Discount not found'

        weight_element = soup.find('td', string='Item Weight')
        weight = weight_element.find_next_sibling('td').get_text(strip=True) if weight_element else 'Weight not found'

        brand_element = soup.find('span', class_='a-size-base po-break-word')
        brand = brand_element.get_text(strip=True) if brand_element else 'Brand Name not found'
        ...
        laptop_specification = laptop_specification_element.get_text(strip=True) if laptop_specification_element else 'Laptop Specification not found'
        ...
        for key, value in product_data.items():
            product_data[key] = ''.join(char for char in value if char.isprintable())
...
def scrape_delivery_address(soup):
    address_element = soup.find('a', id='nav-global-location-popover-link')
    delivery_info = address_element.get_text(strip=True) if address_element else 'Delivery Address not found'
    
    # Extract pincode
    pincode = ''
    if delivery_info:
        match = re.search(r'(\d{6})', delivery_info)
        if match:
            pincode = match.group(1)

            # Remove text after pincode
            delivery_info = re.sub(r'\s*\d{6}\s*.*', '', delivery_info)

    return delivery_info.strip(), pincode

def scrape_delivery_date(soup):
    date_element = soup.find('span', class_='a-color-base a-text-bold')
    delivery_date = date_element.get_text(strip=True) if date_element else 'Delivery Date not found'
    ...
    for page in range(1, num_pages + 1):
        ...
        if response.status_code == 200:
            ...
            for link in product_links:
                ...
                if laptop_data:
                    laptop_data['Delivery Address'] = scrape_delivery_address(soup)
                    laptop_data['Delivery Date'] = scrape_delivery_date(soup)
...
with open('all_laptop_data1.json', 'w', encoding='utf-8') as json_file:
